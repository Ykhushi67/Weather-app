...
export const api = {
  ...
  reverseGeocode: (lat, lon) =>
    fetch(`${BASE_URL}/weather/reverse-geocode?lat=${lat}&lon=${lon}`).then(handle),
  ...
  getAirQuality: (lat, lon) =>
    fetch(`${BASE_URL}/weather/air-quality?lat=${lat}&lon=${lon}`).then(handle),

  compareCities: (cities) =>
    fetch(
      `${BASE_URL}/weather/compare?cities=${encodeURIComponent(JSON.stringify(cities))}`
    ).then(handle),
